...
torchrun --standalone --nproc_per_node=1 -m scripts.base_train -- --depth=12 --target-param-data-ratio=10.5 --device-batch-size=4 --fp8 --run=$WANDB_RUN
...
torchrun --standalone --nproc_per_node=1 -m scripts.chat_sft -- --device-batch-size=4 --run=$WANDB_RUN
